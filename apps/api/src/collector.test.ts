...
const sample101 =
  '172.68.118.127 - - [09/May/2026:11:59:04 +0800] "GET /dw2026ws HTTP/1.1" 101 913 "-" "Go-http-client/1.1" "221.237.156.241"';
...
test('parseOpenRestyAccessLine returns real x-forwarded-for IP for websocket 101 lines', () => {
  ...
  assert.equal(event?.ip, '221.237.156.241');
...
test('parseOpenRestyAccessLine ignores non-101 lines and missing forwarded IPs', () => {
  ...
  const missingIp = sample101.replace('"221.237.156.241"', '"-"');
...
test('collectOnce supports dry-run without writing snapshots and writes collection runs when persisted', async () => {
  ...
  try {
    ...
    await writeFile(
      ...
      [
        sample101,
        sample101.replace('221.237.156.241', '171.213.246.90'),
